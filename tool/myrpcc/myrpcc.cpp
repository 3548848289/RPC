...
void printServiceInfo(ServiceInfo &serviceInfo)
{
    ...
    for (auto rpcInfo : serviceInfo.rpc_infos_) {
        cout << "rpcInfo[name=" << rpcInfo.rpc_name_ << ",mode=" << rpcInfo.rpc_mode_ 
             << ",req=" << rpcInfo.request_name_      << ",resp=" << rpcInfo.response_name_ 
             << ",rpc_file=" << rpcInfo.cpp_file_name_ << "]" << endl;
    }
}

bool genMySvrFiles(ServiceInfo &serviceInfo)
{
    if (not GenMakefile::Gen())
        return false;
    if (not GenInstall::Gen())
        return false;
    if (not GenBuild::Gen(serviceInfo))
        return false;
    if (not GenMain::Gen(serviceInfo))
        return false;
    if (not GenHandler::Gen(serviceInfo))
        return false;
    if (not GenConf::Gen(serviceInfo))
        return false;
    if (not GenBase::ExecCmd("chmod +x ./build.sh ./install.sh"))
        return false;
    return true;
}
...
int main(int argc, char *argv[])
{
    ...
    if (not IsValidProto()) {
        cout<< RED_BEGIN << proto 
            << " invalid, please check proto file." << COLOR_END << endl;
        return -1;
    }
    vector<string> tokens;
    if (not ProtoSimpleParser::Parse2Token("./proto/" + proto, tokens)) {
        cout<< RED_BEGIN 
            << "proto parse failed, please check proto file." << COLOR_END << endl;
        return -1;
    }
    if (debug)
        printTokens(tokens);
    ...
    if (not genMySvrFiles(serviceInfo)) { //实现文件自动生成

...
public:
    static void PrintTraceInfo(Context &ctx, int stackId, int depth, 
                               bool isLast = false, bool outputIsLog = true) {
        if (stackId <= 0) {
        ...
        message = Common::Strings::StrFormat(
            (char *)"%s[%d]%s%s.%s-[%ldus,%d,%d,%s]", getPrefix(depth, isLast).c_str(), 
            traceInfo.current_id(), servicePrefix.c_str(), traceInfo.service_name().c_str(), 
            traceInfo.rpc_name().c_str(), traceInfo.spend_us(),
    ...
    static void AddTraceInfo(std::string serviceName, std::string rpcName, 
                            int64_t spendTimeUs, int32_t statusCode, std::string message) {
        auto trace_stack = ReqCtx.Get().add_trace_stack();
    ...
    static void MergeTraceInfo(Context &ctx) { // 用于MyRPC框架，合入依赖的rpc接口分布式栈信息。

...
public:
    ...
    bool Set(std::string key, std::string value, int64_t expireTime, std::string &error) {
        ...
        Common::Defer defer([&time_stat, this]() {
            DistributedTrace::AddTraceInfo(
                "Redis", "Set", time_stat.GetSpendTimeUs(), status_code_, message_); 
        });
    ...
    bool Get(std::string key, std::string &value, std::string &error) {
        ...
        Common::Defer defer([&time_stat, this]() { 
            DistributedTrace::AddTraceInfo(
                "Redis", "Get", time_stat.GetSpendTimeUs(), status_code_, message_);
        });
        cmd_.makeGetCmd(key);
        if (not execAndCheck(error))        
            return false;
    ...
    bool Del(std::string key, int64_t &delCount, std::string &error)
    {
        ...
        Common::Defer defer([&time_stat, this]() { 
            DistributedTrace::AddTraceInfo(
                "Redis", "Del", time_stat.GetSpendTimeUs(), status_code_, message_); 
        });
        cmd_.makeDelCmd(key);
        if (not execAndCheck(error))
            return false;
    ...
    bool Incr(std::string key, int64_t &value, std::string &error) {
        ...
        Common::Defer defer([&time_stat, this]() { 
            DistributedTrace::AddTraceInfo(
                "Redis", "Incr", time_stat.GetSpendTimeUs(), status_code_, message_); 
        });

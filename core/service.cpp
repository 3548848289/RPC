...
namespace Core {
    int MyRPCService::Init(int argc, char *argv[]) {
        ...
        std::string cfgFile = Common::Strings::StrFormat(
            (char *)"/home/backend/service/%s/%s.conf", str, str);
        assert(config_.Load(cfgFile)); // 加载配置文件
        config_.Dump([](const std::string &section, 
    ...
    void MyRPCService::Run() {
        if (not Common::ServiceLock::lock(
            "/home/backend/lock/subsys/" + Common::Utils::GetSelfName())) {
        ...
        for (int64_t i = 0; i < processCount; i++) {
            pid_t pid = fork();
            if (pid < 0) {
                ERROR("call fork failed. errMsg[%s]", strerror(errno)); continue;
            }
            if (0 == pid) { // 子进程直接跳出循环
                is_master_ = false; break;
            }
            pids_.push_back(pid);
        }
        if (is_master_)
    ...
    void MyRPCService::monitorWorker() {
        while (true) {
            ...
            for (size_t i = 0; i < pids_.size(); i++)
                if (pids_[i] <= 0 || kill(pids_[i], 0) != 0) // 进程不存在或无法访问，kill 会返回非零值

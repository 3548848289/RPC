...
public:
    static bool lock(std::string pidFile) {
        ...
        if (fd < 0) {
            ERROR("open %s failed, error:%s", pidFile.c_str(), strerror(errno)); return false;
        }
        int ret = lockf(fd, F_TEST, 0); // 返回0表示未加锁或者被当前进程加锁；返回-1表示被其他进程加锁
        if (ret < 0) {
            ERROR("lock %s failed, error:%s", pidFile.c_str(), strerror(errno)); return false;
        }
        ret = lockf(fd, F_TLOCK, 0); // 尝试给文件加锁，加锁失败直接返回错误码，而不是一直阻塞
        if (ret < 0) {
            ERROR("lock %s failed, error:%s", pidFile.c_str(), strerror(errno)); return false;
        }
        ret = ftruncate(fd, 0); // 清空文件
        if (ret < 0) {
            ERROR("ftruncate failed, error:%s", strerror(errno)); return false;
        }

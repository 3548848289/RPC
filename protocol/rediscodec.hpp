...
class RedisCodec : public Codec { // 协议编解码
public:
    RedisCodec() { 
        pkt_.Alloc(100);
    }
...
private:
    bool decodeFirstChar(uint8_t **data, uint32_t &needDecodeLen, 
    ...
    bool decodeSimpleValue(uint8_t **data, uint32_t &needDecodeLen, 
                           uint32_t &decodeLen, bool &decodeBreak){
        char *curData = (char *)(*data);
        bool getValue = false;
    ...
    bool decodeBulkValue(uint8_t **data, uint32_t &needDecodeLen, 
                         uint32_t &decodeLen, bool &decodeBreak){
        char *curData = (char *)(*data);
        int32_t bulkLen = 0;
